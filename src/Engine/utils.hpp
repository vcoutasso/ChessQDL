...
namespace chessqdl {
	...
	/**
	 * @brief This method returns the equivalent position of the set bit to the board. For example 0x01 would return a1, 0x02 a2 and so on.
	 * @param pos  bitboard (preferably with only one bit set)
	 ...
	 */
	int evaluateBoard(const BitboardArray &board, enumColor color);


	/**
	 * @brief Method to read an integer from stdin in a clean and sanitized way.
	 ...
	 * @return Index of the least significant bit currently set
	 */
	int leastSignificantSetBit(uint64_t value);

	/**
	 * @brief Constructs the name of the move that is formed by moving a piece
